...
# turbo firmware. Result: download throughput ~145 KB/s -> ~1 MB/s.
#
# Why: the stock prebuilt lwIP library ships TCP_SND_BUF = 2*MSS (2.9 KB in
# flight per round trip) which caps GET speed. We rebuild with 16*MSS.
#
...
#   liblwip2-1460-feat-16xMSS.a  -> copy to
...
edits = [
 (r'#define TCP_SND_BUF\s+\(2 \* TCP_MSS\)', '#define TCP_SND_BUF                     (16 * TCP_MSS) /* SDWIFI-turbo: was 2*MSS */'),
